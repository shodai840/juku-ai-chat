...
// 会話履歴（生徒がブラウザを閉じた後でも見返せるように、ログイン中の生徒アカウントに紐づけて保存する）
export async function insertMessages(rows) {
  const res = await supabaseRequest('/messages', {
    method: 'POST',
    body: JSON.stringify(rows)
  });
  if (!res.ok) {
    const text = await res.text().catch(() => '');
    throw new Error(`Supabase会話履歴保存エラー: ${res.status} ${text}`);
  }
}

// 指定した生徒の会話履歴を新しい順に取得する（beforeを指定すると、それより古いものだけを取得）。
// 呼び出し側でlimit+1件要求し、実際にlimit件を超えて返ってきたかどうかでhasMoreを判定する想定。
export async function getMessagesForStudent(studentId, { limit, before }) {
  let path = `/messages?student_id=eq.${encodeURIComponent(studentId)}&select=id,role,text,created_at&order=created_at.desc&limit=${encodeURIComponent(limit)}`;
  if (before) {
    path += `&created_at=lt.${encodeURIComponent(before)}`;
  }
  const res = await supabaseRequest(path);
  if (!res.ok) throw new Error(`Supabase会話履歴取得エラー: ${res.status}`);
  return res.json();
}
